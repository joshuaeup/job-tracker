...
const passesTitle = (title: string): boolean =>
  TITLE_ALLOWLIST_PATTERNS.some((re) => re.test(title));
...
const passesLocation = (job: NormalizedJob): boolean => {
  ...
  // international office roles as isRemote:true (e.g. Ashby Hyderabad)
...
const passesSeniority = (title: string): boolean =>
  !SENIORITY_BLOCKLIST_PATTERNS.some((re) => re.test(title));
 ...
 * obvious mismatches only.
 *
 * @returns The subset of jobs that pass all active filters.
 */
export const filter = (jobs: NormalizedJob[]): NormalizedJob[] => {
