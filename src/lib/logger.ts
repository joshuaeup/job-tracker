...
const formatStage = (stage: string): string =>
  `${COLOR.bold}[${stage.toUpperCase().padEnd(7)}]${COLOR.reset}`;

const formatMessage = (stage: string, level: Level, message: string): string =>
  `${COLOR.gray}[${timestamp()}]${COLOR.reset} ${formatStage(stage)} ${LEVEL_FORMAT[level]} ${message}`;
...
/**
 * Creates a logger scoped to a pipeline stage (e.g. "FETCH", "NORMALIZE").
 * Output format: `[HH:MM:SS] [STAGE  ] LEVEL  message`
 *
 * @param stage - Short stage identifier shown in every log line.
 */
export const createLogger = (stage: string): Logger => ({
  info: (message: string): void => {
